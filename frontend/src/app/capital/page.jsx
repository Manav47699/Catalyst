...
export default function StartupsPage() {
  ...
  const [activeTab, setActiveTab] = useState('discover-startups');
  ...
  useEffect(() => {
    fetch("http://localhost:8000/capital/startups/")
      .then(async (res) => {
        if (!res.ok) {
          const text = await res.text();
          throw new Error(`Error fetching startups: ${text}`);
        }
        return res.json();
      })
  ...
  const handleMakeRequest = (startup) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.address || !formData.gmail) {
      alert("Please fill in required fields (address & email)");
      return;
    }
    ...
    try {
      const createRes = await fetch(
        `http://localhost:8000/capital/startups/${selectedStartup.id}/create/`,
        {
          method: "POST",
      ...
      const startupRequestId = createData.startup_request_id;

      const checkoutRes = await fetch(
        "http://localhost:8000/capital/stripe/create-checkout-session/",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ startup_request_id: startupRequestId }),
        }
      );
        ...
        <div className="bg-white/80 backdrop-blur-md shadow-lg border-b-4 border-green-400">
          <div className="max-w-4xl mx-auto px-6 py-8 text-center">
            ...
            <p className="text-gray-700 text-lg">
              Invest in ideas shaping the future
            </p>
          </div>
        </div>
              ...
              >
                <div className="p-6">
                  <div className="flex items-start gap-4 mb-4">
                    <img
                      src={startup.profilepic || "/default-startup.jpg"}
                      alt={startup.name}
                      onClick={() => setExpandedImage(startup.profilepic || "/default-startup.jpg")}
                    ...
                    <div className="flex-1">
                      <h3 className="text-2xl font-bold text-green-600 mb-1">{startup.name}</h3>
                      <p className="text-gray-700 text-sm mb-2">{startup.about_us}</p>
                      
                      {/* Benefits */}
                      {startup.benefits && (
                        <p className="text-gray-800 text-sm mb-3 border-l-4 border-green-400 pl-3 bg-green-50 py-2 rounded-r-lg">
                          {startup.benefits}
                        </p>
                      )}

                      <div className="inline-flex items-center bg-green-100 text-green-800 px-3 py-1 rounded-full font-semibold text-sm">
                        Minimum: ${startup.minimum_donation}
                      </div>
                    </div>
                  </div>

                  <button
                    onClick={() => handleMakeRequest(startup)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-gradient-to-r from-green-500 to-emerald-500 text-white p-6 rounded-t-2xl flex justify-between items-center">
              <div>
                <h2 className="text-2xl font-bold">Support Startup</h2>
                <p className="text-green-100 text-sm mt-1">{selectedStartup.name}</p>
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="mt-6 flex gap-3">
                ...
                >
                  {loading ? "Processing..." : `Donate $${selectedStartup.minimum_donation}`}
                </button>
              </div>
            </div>
          </div>
